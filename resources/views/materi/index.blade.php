...
<div class="space-y-8">
    ...
    {{-- ── FLASH MESSAGES ── --}}
    @if(session('success'))
        <div class="bg-green-100 text-green-700 px-5 py-3 rounded-2xl text-sm font-semibold">
            {{ session('success') }}
        </div>
    @endif
    ...
    <div class="bg-white rounded-3xl shadow-sm p-6 lg:p-8">
        <h2 class="text-lg font-bold text-[#1B254B] mb-6">Upload Materi Pembelajaran</h2>
            ...
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <label class="flex flex-col items-center justify-center h-32 mt-2 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-blue-500 transition">
                        ...
                        <span class="text-xs text-gray-400 mt-2">PDF, Word, Excel, Video, JPG (maks 50MB)</span>
    ...
    <div class="bg-white rounded-3xl shadow-sm p-6 lg:p-8">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-lg font-bold text-[#1B254B]">Daftar Materi</h2>
            <span class="text-xs text-gray-400">{{ count($materi) }} materi tersedia</span>
        </div>

        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead>
                    <tr class="text-left text-gray-400 border-b text-[11px] uppercase tracking-widest">
                ...
                <tbody class="text-[#1B254B]">
                    ...
                    <tr class="border-b hover:bg-gray-50 transition">
                        <td class="py-4 font-semibold">
                            {{ $m->judul }}
                            @if($m->deskripsi)
                                <p class="text-xs text-gray-400 font-normal mt-0.5">{{ Str::limit($m->deskripsi, 60) }}</p>
                            @endif
                        </td>
                        ...
                        <td class="py-4">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="text-xs uppercase font-semibold text-gray-500">{{ $m->tipe_file }}</span>
                            </div>
                        </td>
                        ...
                        <td class="py-4">
                            <div class="flex items-center justify-end space-x-2">
                                ...
                                <form method="POST" action="{{ route('materi.destroy', $m) }}"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                        class="inline-flex items-center space-x-1 bg-red-50 text-red-600 hover:bg-red-100 px-3 py-1.5 rounded-lg text-xs font-semibold transition">
                    ...
                    <tr>
                        <td colspan="4" class="py-10 text-center text-gray-400 text-sm">
                            Belum ada materi. Upload materi pertama kamu!
                        </td>
                    </tr>
                    @endforelse
                </tbody>
